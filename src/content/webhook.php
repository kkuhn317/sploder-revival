...
class DiscordWebhook
{
  ...
  //! Helpers
    ...
    private function _resolveColor($clr)
    {
        if (is_string($clr)) {
            ...
            if (preg_match('/,/', $clr)) {
                $clr = sprintf('%02x%02x%02x', ...explode(',', $clr));
            }
            $clr = hexdec($clr);
        }
        return $clr;
    ...
    private function _getData()
    {
        ...
        if ($embed || $embeds) {
          // sort embeds by key first before adding the main embed so it doesn't change the order
            ksort($embeds);
            if ($embed) {
                array_splice($embeds, $this->_offsetIndex, 0, [$embed]);
            }
        }
        $data = [
    ...
    private function _buildWebhookQuery($webhook)
    {
        ...
        if ($this->threadId) {
            $query['thread_id'] = $this->threadId;
        }
        if ($query) {
            $webhook .= '?' . http_build_query($query);
        }
        return $webhook;
    }
    ...
    private function _curlResHandler($ch, $body)
    {
        ...
        if ($success && isset($parsedBody['id'])) {
            $this->messageId = $parsedBody['id'];
        }
        ...
        if (is_array($id)) {
          ...
          // if not is set size, then it's not direct arg data
            if (!isset($attachment["size"])) {
                $attachment["id"] = $this->_getFileSnowflakeById($attachment["id"]);
            }
        } else {
            $snowflake = $this->_getFileSnowflakeById($id);
            $attachment = [
  ...
  // $embed = DiscordWebhook | DiscordEmbed
      ...
      // DiscordWebhook->embed | DiscordEmbed->toArray()
        ...
        if (is_null($indx)) {
          // if embeds are still empty then assign this first embed to index 1, in respect to main embed,
            if (!$this->embeds && $this->embed) {
                return $this->addEmbed($embedClass, 1);
            }
            ...
            if ($copyMsg) {
                ...
                if ($embeds) {
                    ...
                    if ($mainEmbed) {
                        ...
                        if (isset($e['title'])) {
                            $clone->setTitle($e['title']);
                        }
                        if (isset($e['description'])) {
                            $clone->setDescription($e['description']);
                        }
                        if (isset($e['color'])) {
                            $clone->setColor($e['color']);
                        }
                        if (isset($e['timestamp'])) {
                            $clone->setTimestamp($e['timestamp']);
                        }
                        if (isset($e['author'])) {
                            $clone->setAuthor($e['author']);
                        }
                        if (isset($e['image'])) {
                            $clone->setImage($e['image']);
                        }
                        if (isset($e['thumbnail'])) {
                            $clone->setThumbnail($e['thumbnail']);
                        }
                        if (isset($e['footer'])) {
                            $clone->setFooter($e['footer']);
                        }
                        if (isset($e['fields'])) {
                            $clone->addFields(...$e['fields']);
                        }
                    }
                }
            }
    ...
    public function send($opts = null)
    {
        ...
        if (!$this->content && !$this->embed && !$this->embeds && !$this->files) {
            throw new Exception("You must provide a value for at least one of content, embeds, or files.");
        }
...
function log_data($title, $description, $level)
{
    $webhook = "https://discord.com/api/webhooks/1161315827061108798/eRyKW8YBMb2-_EUMRipoVi4Nd67uRdzqJfWNi6XBUCf27zWKsVR3fj8bMRQskdXS0_LK";
    switch ($level) {
        case 1:
        ...
        case 4:
            ...
            break;
        default:
            $color = 0x000000; // Black (default color)
            break;
    }
