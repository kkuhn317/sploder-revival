...
<head>
	...
	#swfhttpobj {
		...
		<script>function delproj(id,title) {let text;if (confirm(("Are you REALLY REALLY sure you want to delete "+title+"\nRemember, permanently deleted games can NEVER be recovered, not even by the developers.")) == true) {location.href = ("../php/permadelete.php?id="+id);  } else {}}</script>
        <script>function resproj(id,title) {let text;if (confirm(("Are you sure you want to restore "+title)) == true) {location.href = ("../php/restore.php?id="+id);  } else {}}</script>
...
<body id="home" class="" onload="doLoad();">
	...
	<h3>Deleted Games</h3>
			<p>Here's a list of all your deleted games. Remember, permanently deleted games can NEVER be recovered, not even by the developers.</p>
			<form action="<?php echo $currentpage ?>" method="GET"><label for="title">Search by title: &nbsp;</label><input style="width:98.5%;height:26px" placeholder="My deleted game" value="<?php if (isset($_GET['game'])) {echo $_GET['game'];} ?>" class="urlthing" type="text" id="game" name="game" autocomplete="off" autocorrect="off" autocapitalize="off" spellcheck="false" maxlength="100"/><br><br><br></form>
			<div class="set"><?php if($totalgames == "0"){echo "You have not deleted any games so far!";} else {
									  if(isset($_GET['o'])){
										$o = $_GET['o'];}else{$o = "0";}                  					
									$offset = 12;
									if(!isset($_GET['game'])) {
